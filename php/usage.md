...
docker run -di -p 3308:3306 --network www-net --name c-mysql -h mysql -v "/Users/datdao/Developer/Code/docker-exercise/php/mycode/db":/var/lib/mysql -e MYSQL_ROOT_PASSWORD=abc123  mysql
...
docker exec -it c-mysql bash
  ...
  CREATE USER 'dat'@'%' IDENTIFIED BY 'abc123';
  GRANT ALL PRIVILEGES ON wp_blog.* TO 'dat'@'%';
mysql --host=172.19.0.4 --port=3308 -u root -pabc123 # failed khong the connect
```